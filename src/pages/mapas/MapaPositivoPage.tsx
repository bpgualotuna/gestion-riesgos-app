/**
 * Mapa de Riesgos Positivo Page
 * Interactive 5x5 risk matrix visualization for positive risks (opportunities)
 */
...
export default function MapaPage() {
  ...
  // FORZAR clasificación POSITIVA para este mapa (no permitir cambio)
  const [clasificacion] = useState<string>(CLASIFICACION_RIESGO.POSITIVA);
  ...
  const puntosFiltrados = useMemo(() => {
    // 🔍 DEBUG: Log inicial
    console.log('=== DIAGNÓSTICO FILTRADO DE PUNTOS ===');
    console.log('Puntos recibidos del backend:', puntos);
    console.log('Total puntos:', puntos?.length || 0);
    console.log('Riesgos completos:', riesgosCompletos.length);
    console.log('Clasificación buscada:', clasificacion);
    console.log('CLASIFICACION_RIESGO.POSITIVA:', CLASIFICACION_RIESGO.POSITIVA);
    console.log('Filtro área:', filtroArea);
    console.log('Filtro proceso:', filtroProceso);
    console.log('Procesos propios:', procesosPropios.length);
    
    if (!puntos || puntos.length === 0) {
      console.log('⚠️ No hay puntos del backend');
      return [];
    }
    ...
    if (filtroProceso && filtroProceso !== 'all') {
      ...
      if (procesoFiltrado) {
        ...
        console.log('Filtrando por proceso específico:', filtroProceso);
      } else {
        // Si el proceso filtrado no está asignado al usuario, no mostrar nada
        console.log('⚠️ Proceso filtrado no está asignado al usuario');
        return [];
      }
    } 
    // Si hay filtro de área, filtrar procesos por área (pero solo de los asignados)
        ...
        .map(p => String(p.id));
        console.log('Filtrando por área:', filtroArea, 'Procesos:', procesosIds);
    }
    // Si no hay filtros de área ni proceso, usar procesos asignados del usuario
    else {
      procesosIds = procesosPropios.map((p) => String(p.id));
      console.log('Sin filtros, usando todos los procesos propios:', procesosIds);
    }
    ...
    const filtrados = puntos.filter((p) => {
      ...
      if (!riesgo) {
        console.log(`Punto ${p.riesgoId}: Riesgo no encontrado`);
        return false; // Riesgo no encontrado, excluir
      }
      
      console.log(`Punto ${p.riesgoId}: Riesgo encontrado, clasificación="${riesgo.clasificacion}"`);
      ...
      if (procesosIds.length > 0) {
        ...
        if (!procesosIds.includes(procesoIdRiesgo)) {
          console.log(`Punto ${p.riesgoId}: No pertenece a procesos asignados`);
          return false; // No pertenece a los procesos asignados/filtrados
        }
      } else {
        // Si no hay procesos asignados, no mostrar nada
        console.log('⚠️ No hay procesos asignados');
        return false;
      }
      
      // Negativos/heredados: todo lo que no es oportunidad explícita (null, Excel, negativa). Positivos: solo consecuencia positiva.
      // IMPORTANTE: Aceptar AMBOS formatos de clasificación positiva
      const esClasificacionPositiva = 
        riesgo.clasificacion === CLASIFICACION_RIESGO.POSITIVA || 
        riesgo.clasificacion === 'Positiva' ||
        riesgo.clasificacion?.toLowerCase().includes('positiv');
      
      if (clasificacion === CLASIFICACION_RIESGO.NEGATIVA) {
        if (esClasificacionPositiva) {
          console.log(`Punto ${p.riesgoId}: Es positivo pero buscamos negativos`);
          return false;
        }
      } else if (!esClasificacionPositiva) {
        console.log(`Punto ${p.riesgoId}: No es positivo (clasificación="${riesgo.clasificacion}")`);
        return false;
      }

      console.log(`✓ Punto ${p.riesgoId}: INCLUIDO`);
      return true;
    });
    
    console.log('Total puntos filtrados:', filtrados.length);
    console.log('Puntos filtrados:', filtrados);
    console.log('=======================================');
  ...
  const matrizInherente = useMemo(() => {
    ...
    console.log('=== DIAGNÓSTICO MAPA POSITIVO - INHERENTE ===');
    console.log('Total puntos filtrados:', puntosFiltrados.length);
    ...
    puntosFiltrados.forEach((punto) => {
      ...
      console.log(`Riesgo ${punto.riesgoId}: prob=${probabilidad}, imp=${impacto}, clave=${clave}`);
    ...
    console.log('Matriz inherente generada:', matriz);
    console.log('Total de celdas con riesgos:', Object.keys(matriz).length);
    console.log('===========================================');
  ...
  const getCellColor = (probabilidad: number, impacto: number): string => {
    ...
    // Mapa positivo (oportunidades): escala de azules según imagen
    // Mayor frecuencia + mayor impacto = mejor oportunidad = azul más oscuro
    if (esOportunidades) {
      // Azul muy oscuro (Navy) - Impacto 5, Frecuencia 4-5
      if (imp === 5 && (prob === 4 || prob === 5)) {
        return '#003366'; // Navy blue - Alta oportunidad
      }
      
      // Azul medio - Impacto 5, Frecuencia 3
      if (imp === 5 && prob === 3) {
        return '#0099CC'; // Medium blue
      }
      
      // Azul claro - Impacto 4, Frecuencia 3-5
      if (imp === 4 && prob >= 3) {
        return '#33B5E5'; // Light blue
      }
      
      // Azul muy claro - Impacto 3, Frecuencia 5
      if (imp === 3 && prob === 5) {
        return '#66CCFF'; // Very light blue
      }
      
      // Gris medio - Resto de combinaciones con algún valor
      if ((imp === 3 && prob >= 3 && prob <= 4) || 
          (imp === 2 && prob >= 3) ||
          (imp === 1 && prob >= 4)) {
        return '#999999'; // Medium gray
      }
      
      // Gris claro - Combinaciones bajas
      if ((imp === 2 && prob <= 2) || 
          (imp === 1 && prob <= 3)) {
        return '#CCCCCC'; // Light gray
      }
      
      // Blanco/muy claro - Impacto 1, Frecuencia 1-2
      if (imp === 1 && prob <= 2) {
        return '#F5F5F5'; // Almost white
      }
      
      // Default gris para cualquier otra combinación
  ...
  const getColorByNivelRiesgo = (nivelRiesgo: string | null | undefined): string => {
    ...
    if (esOportunidades) {
      // Mapeo de niveles a colores para oportunidades (escala de azules)
      if (
        nivelUpper.includes('ALTA') ||
        nivelUpper.includes('EXTREMO') ||
        nivelUpper.includes('MUY ALTA')
      ) {
        return '#003366'; // Navy blue - Alta oportunidad
      }
      if (nivelUpper.includes('CRÍTICO') || nivelUpper.includes('CRITICO')) {
        return '#0099CC'; // Medium blue - Oportunidad crítica
      }
      if (nivelUpper.includes('MODERADA') || nivelUpper.includes('MEDIO')) {
        return '#33B5E5'; // Light blue - Oportunidad moderada
      }
      if (nivelUpper.includes('BAJA') || nivelUpper.includes('BAJO')) {
        return '#CCCCCC'; // Light gray - Oportunidad baja
      }
      return '#E0E0E0'; // Gris - Sin clasificar
    }
  ...
  // Variables para los filtros personalizados
      ...
      {!sinAsignaciones && (
        ...
        <Box sx={{ mb: 3 }}>
            <Typography variant="h4" gutterBottom fontWeight={700} sx={{ color: '#1976d2' }}>
              Mapas de Calor de Riesgos Positivos (Oportunidades)
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Solo riesgos con consecuencia positiva. La evaluación inherente usa la lógica de oportunidades.
            </Typography>
        </Box>
        ...
        <Grid2 container spacing={3}>
          ...
          <Grid2 xs={12}>
            {/* Filtros personalizados sin selector de clasificación */}
            <Card sx={{ mb: 2 }}>
              <CardContent>
            ...
            {/* Leyenda de colores para riesgos positivos */}
            <Card sx={{ mb: 2 }}>
              <CardContent sx={{ pt: 2, pb: 2 }}>
                <Typography variant="subtitle2" fontWeight={600} gutterBottom>
                  Leyenda de niveles (Oportunidades)
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Mayor frecuencia + mayor impacto = mejor oportunidad (azul más oscuro)
                </Typography>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    />
                    <Typography variant="body2">Muy Alta (Navy)</Typography>
                  </Box>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Box
                      sx={{
                    ...
                    />
                    <Typography variant="body2">Alta (Azul Medio)</Typography>
                  </Box>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Box
                      sx={{
                    ...
                    />
                    <Typography variant="body2">Moderada (Azul Claro)</Typography>
                  </Box>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Box
                      sx={{
                    ...
                    />
                    <Typography variant="body2">Media (Celeste)</Typography>
                  </Box>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Box
                      sx={{
                    ...
                    />
                    <Typography variant="body2">Baja (Gris Medio)</Typography>
                  </Box>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Box
                      sx={{
                    ...
                    />
                    <Typography variant="body2">Muy Baja (Gris Claro)</Typography>
                  </Box>
                </Box>
              </CardContent>
            </Card>
