...
function RiesgoFormulario({
  ...
  const { esGerenteGeneralProceso } = useAuth();
  const tipoRiesgoSeleccionado = useMemo(() => {
    return TIPOS_RIESGOS.find(t => t.codigo === riesgo.tipoRiesgo) || null;
  }, [riesgo.tipoRiesgo]);
  
  // Filtrar tipos de riesgo para Gerente General en modo Proceso
  const tiposRiesgosDisponibles = useMemo(() => {
    if (esGerenteGeneralProceso) {
      return TIPOS_RIESGOS.filter(t => t.codigo === '01 Estratégico');
    }
    return TIPOS_RIESGOS;
            ...
            <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: 2, p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <TextField
                  ...
                  disabled={isReadOnly || esGerenteGeneralProceso}
                                onChange={(e) => actualizarRiesgo(riesgo.id, { tipoProceso: e.target.value })}
                                sx={{ fontSize: '0.875rem' }}
                                disableUnderline
                              >
                                <MenuItem value="01 Estratégico">01 Estratégico</MenuItem>
                                {!esGerenteGeneralProceso && <MenuItem value="02 Operacional">02 Operacional</MenuItem>}
                                {!esGerenteGeneralProceso && <MenuItem value="03 Apoyo">03 Apoyo</MenuItem>}
                              </Select>
                            </FormControl>
                          </TableCell>
                        </TableRow>
                    ...
                    <TableContainer component={Paper} variant="outlined">
                      <Table size="small">
                        <TableBody>
                          ...
                          <TableCell>
                          <Autocomplete
                            value={riesgo.tipoRiesgo ? tiposRiesgosDisponibles.find(t => t.codigo === riesgo.tipoRiesgo) || null : null}
                            onChange={(_, newValue) => {
                              if (newValue) {
                            ...
                            options={tiposRiesgosDisponibles}
            ...
            <CardContent sx={{ flexGrow: 1, p: 3 }}>
              ...
              disabled={isReadOnly}
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                      ...
                      <FormControl size="small" sx={{ minWidth: 100 }}>
                        <Select
                          value={riesgo.impactos.confidencialidad || 1}
                          onChange={(e) => {
                            const val = Number(e.target.value);
                            if (!isNaN(val) && val >= 1 && val <= 5) {
                              actualizarRiesgo(riesgo.id, {
                                impactos: { ...riesgo.impactos, confidencialidad: val }
                              });
                            }
                          }}
              disabled={isReadOnly}
                        >
                          {[1, 2, 3, 4, 5].map((val) => (
                            <MenuItem key={val} value={val}>
                              <Tooltip 
                                title={DESCRIPCIONES_IMPACTO.confidencialidad[val] || ''}
                                arrow
                                placement="left"
                              >
                                <Box component="span" sx={{ width: '100%' }}>
                    ...
                    {(() => {
                      const valor = riesgo.impactos.confidencialidad && !isNaN(Number(riesgo.impactos.confidencialidad)) ? Number(riesgo.impactos.confidencialidad) : 1;
                      return (
                        <Typography variant="caption" color="text.secondary" sx={{ whiteSpace: 'pre-line' }}>
                          {valor} - {DESCRIPCIONES_IMPACTO.confidencialidad[valor] || ''}
                          </Typography>
                      );
                    })()}
                  </Box>
