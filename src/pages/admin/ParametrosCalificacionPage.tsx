...
function SubtiposCatalog({
    ...
    const subtipos = useMemo(() => {
        ...
        return (tipo?.subtipos || []).map((s) => ({ id: s.id, nombre: s.nombre, descripcion: s.descripcion }));
    ...
    const handleSaveSubtipo = (item: any) => {
        const newTipos = tiposRiesgo.map((t) => {
            ...
            const existe = subtiposActuales.some((s) => s.id === item.id);
            const nuevosSubtipos: SubtipoRiesgo[] = existe
                ? subtiposActuales.map((s) => (s.id === item.id ? { id: item.id, nombre: item.nombre, descripcion: item.descripcion } : s))
                : [...subtiposActuales, { id: item.id, nombre: item.nombre, descripcion: item.descripcion }];
            return { ...t, subtipos: nuevosSubtipos };
        });
        onSaveTiposRiesgo(newTipos);
    };

    const handleDeleteSubtipo = (id: number) => {
        if (!window.confirm('¿Confirma eliminación del subtipo?')) return;
        const newTipos = tiposRiesgo.map((t) => {
            if (t.codigo !== tipoSeleccionado) return t;
            return { ...t, subtipos: (t.subtipos || []).filter((s) => s.id !== id) };
        });
        onSaveTiposRiesgo(newTipos);
    };
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <SimpleCatalog
                ...
                defaultItem={{ id: 0, nombre: '', descripcion: '' }}
...
export default function ParametrosCalificacionPage() {
    ...
    const filteredFrecuencias = useMemo(() => 
        frecuencias.filter(f => f.frecuencia.toLowerCase().includes(searchCausas.frecuencias.toLowerCase())),
        ...
        <Box sx={{ p: 3, bgcolor: '#f5f5f5', minHeight: '100vh' }}>
            <Box sx={{ mb: 4 }}>
                <Typography variant="h4" gutterBottom fontWeight={700}>
                    Parámetros de Calificación e Identificación
                </Typography>
                <Typography variant="body1" color="text.secondary">
                    Gestión centralizada de los catálogos y parámetros utilizados en la identificación y calificación de riesgos.
                </Typography>
            </Box>
            ...
            <Paper sx={{ bgcolor: 'white', borderRadius: '8px', overflow: 'hidden' }}>
                ...
                >
                    ...
                    />
                    <Tab 
                        icon={<ProbabilityIcon sx={{ fontSize: 24 }} />}
                        iconPosition="top"
                        label="Causas" 
                    />
                    <Tab 
                    ...
                    />
                    <Tab 
                        icon={<GradingIcon sx={{ fontSize: 24 }} />}
                        iconPosition="top"
                        label="Calificación" 
                    />
                </Tabs>
            ...
            <TabPanel value={tabValue} index={1}>
                ...
                <TabPanel value={subTabValue[1]} index={1}>
                    ...
                    <SimpleCatalog
                        ...
                        columns={[
                            { field: 'id', headerName: 'ID', width: 80 },
                            { field: 'frecuencia', headerName: 'Frecuencia', width: 160 },
                            { field: 'descripcion', headerName: 'Descripción', flex: 1 }
                        ]}
